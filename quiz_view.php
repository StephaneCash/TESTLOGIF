...
<body>
    ...
    <div class="container-fluid admin">
        ...
        <button class="btn btn-primary bt-sm" id="new_question"><i class="fa fa-plus"></i> Ajouter question</button>
        <button class="btn btn-primary bt-sm" id="new_student"><i class="fa fa-plus"></i> Ajouter candidat</button>
        <br>
        <br>
            ...
            <div class="card-body">
                <ul class="list-group">
                    ...
                    <li class="list-group-item"><?php echo ucwords($row['name']) ?>
                        <button class="btn btn-sm btn-outline-danger remove_student pull-right"
                            data-id="<?php echo $row['id'] ?>" data-qid='<?php echo $row['qid'] ?>' type="button"><i
            ...
            <div class="modal-dialog modal-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModallabel">Ajout d'une nouvelle question</h4>
                            ...
                            <div class="form-group">
                                ...
                                <span>
                                    <label><input type="radio" name="is_right[0]" class="is_right" value="1">
                                        <small>Question réponse</small></label>
                                </span>
                                <br>
            ...
            <div class="modal-dialog modal-centered" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title" id="myModallabel">Ajouter nouveau(x) candidat(s)</h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                                aria-hidden="true">&times;</span></button>
                    </div>
                    <form id='student-frm'>
...
$(document).ready(function() {
    ...
    $('#new_question').click(function() {
        $('#msg').html('')
        $('#manage_question .modal-title').html('Ajout d\'une nouvelle question')
        ...
        $.ajax({
            ...
            success: function(resp) {
                if (typeof resp != undefined) {
                    ...
                    $('#manage_question .modal-title').html('Editer la Question')
                    $('#manage_question').modal('show')
    ...
    $('.remove_question').click(function() {
        var id = $(this).attr('data-id')
        var conf = confirm('Etes-vous sûr de supprimer ?');
        if (conf == true) {
            $.ajax({
    ...
    $('#question-frm').submit(function(e) {
        ...
        $('#question-frm [name="submit"]').html('Enregistrer...')
        $('#msg').html('')
        ...
        $.ajax({
            ...
            success: function(resp) {
                if (resp == 1) {
                    alert('Opération a réussie avec succès');
                    location.reload()
                }
            }
        })
    ...
    $('#student-frm').submit(function(e) {
        ...
        $.ajax({
            ...
            error: err => {
                ...
                alert('An error occured')
                $('#quiz-frm [name="submit"]').removeAttr('disabled')
                $('#quiz-frm [name="submit"]').html('Enregistrer')
            },
            success: function(resp) {
                if (resp == 1) {
